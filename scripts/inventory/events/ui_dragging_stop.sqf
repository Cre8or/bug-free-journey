...
case "ui_dragging_stop": {
	...
	if (!isNull _draggedCtrl) then {
		...
		([_itemData, _targetContainerData, _slotPos, _slotSize] call cre_fnc_canFitItem) params ["_canFit"];
		...
		if (_canFit) then {
			...
			["ui_item_move", [_draggedCtrl, _slotPos, _targetContainerCtrl]] call cre_fnc_inventory;
			...
			private _EH = _inventory displayAddEventHandler ["MouseButtonUp", {
				["ui_dragging_complete", _this] call cre_fnc_inventory;
			}];
